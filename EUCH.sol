...
contract EUCH is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    function bulkMint(address to, string memory uri, uint tipe, uint amount) public onlyAdmin{
        for(uint i=0; i<amount; i++){
            uint256 tokenId = _tokenIdCounter.current()+1;
            string memory urii=string(abi.encodePacked(uri,tokenId));
    ...
    function safeMint(address to, string memory uri, uint tipe, address chestaddress) public {
